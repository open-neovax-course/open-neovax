...
def _make_candidate(candidate_id: str = "TEST_01", peptide_wt: str = "AAAAAAAAA", peptide_mut: str = "AAAAAAAAV", mut_pos_1based: int = 9) -> Candidate:
    return Candidate(
...
def test_invalid_case():
    ...
    candidate = _make_candidate(peptide_wt="111111111", peptide_mut="111111111",mut_pos_1based=9)
    name, value = get_score(candidate)
    assert isinstance(value, (int, float))
...
def test_invalid_case_mut_pos_1based():
    ...
    candidate = _make_candidate(mut_pos_1based=18,peptide_wt="AAAAAAAAAAAAAAAAAA", peptide_mut="AAAAAAAAAAAAAAAAAV")
    name, value = get_score(candidate)
    assert isinstance(value, (int, float))
    assert math.isnan(value)
    assert math.isinf(value)
