"""Tests for module B2 - TAP transport scoring module(B2)"""
...
def test_favorable_cterminus_gives_positive_score():
    ...
    assert value > 0.0, "V at the C-terminus is favorable for TAP"
...
def test_unfavorable_cterminus_gives_negative_score():
    ...
    assert value < 0.0, "D at the C-terminus is unfavorable for TAP"
...
def test_proline_cterminus_gives_negative_score():
    ...
    assert value < 0.0, "P at the C-terminus is heavily penalized"


def test_highly_charged_peptide_penalized():
    """Highly charged peptide should be penalized even with good C-terminus."""
    ...
    assert (
        value_charged < value_normal
    ), "A highly charged peptide should score lower than a normal peptide"


def test_moderate_charge_no_penalty():
    """Moderate charge (|q| <= 2) should not incur charge penalty."""
...
def test_consistency_wt_vs_mut():
    ...
    candidate = SimpleNamespace(peptide_wt="SLMAFTIAV", peptide_mut="SLMAFTIAL")
...
def test_score_monotonicity():
    ...
    for i in range(len(scores) - 1):
        assert scores[i] >= scores[i + 1], (
            f"Order not preserved: {peptides[i][0]} ({scores[i]:.3f}) > "
            f"{peptides[i+1][0]} ({scores[i+1]:.3f})"
        )
...
def test_charge_penalty_progressive():
    ...
    for i in range(1, len(scores)):
        if charges[i] > CHARGE_THRESHOLD:
            assert scores[i] <= scores[i - 1], (
                f"Score should decrease: charge {charges[i-1]}→{charges[i]}, "
                f"score {scores[i-1]:.3f}→{scores[i]:.3f}"
            )
...
def test_score_range():
    ...
    for pep in test_peptides:
        ...
        assert (
            -1.0 <= score <= 0.6
        ), f"Score {score:.3f} for {pep} outside bounds [-1.0, 0.6]"
