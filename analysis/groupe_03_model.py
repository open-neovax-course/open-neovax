...
def load_and_preprocess():
    ...
    df_real = pd.read_csv(PATIENT_REAL_CSV) if PATIENT_REAL_CSV.exists() else pd.DataFrame()
    ...
    X_real_clip = df_real[feature_cols].clip(lower=p1_thresholds, axis=1) if not df_real.empty else None
    ...
    return feature_cols, X_train_scaled, y_train, X_zero_scaled, df_zero, X_real_scaled, df_real
...
def evaluate_real_world(models, X_real, df_real):
    ...
    merged = df_real[real_mask].merge(df_raw_real[["candidate_id", "ic50_nm"]], on="candidate_id")
...
if __name__ == "__main__":
    feat_cols, X_train, y_train, X_zero, df_zero, X_real, df_real = load_and_preprocess()
