...
def plot_feature_importance(imp: pd.Series) -> None:
    imp.head(15).plot(kind="barh", figsize=(10, 6), color="#2980b9")
    plt.xlabel("Importance")
    plt.title("Feature Importance — Random Forest (groupe 09)")
    plt.gca().invert_yaxis()
    _save_plot(ANALYSIS_DIR / "groupe09_feature_importance.png")
...
def visualizations(
    ...
    ax.set_title("ROC Curve (cross-validated, patient_one) — groupe 09")
    ax.legend()
    ...
    ax.set_title("Confusion Matrix (hold-out 20%) — groupe 09")
    _save_plot(ANALYSIS_DIR / "groupe09_confusion_matrix.png")
...
def ordinal_regression(
    ...
    print("  Good result: < 0.5 grade off." if mae < 0.5 else "  Moderate result.")
...
def evaluate_real_vs_decoy(model, df: pd.DataFrame, feature_cols: list[str]) -> None:
...
def evaluate_ic50_correlation(
    ...
    if PATIENT_REAL_RAW.exists():
        raw = pd.read_csv(PATIENT_REAL_RAW)[["candidate_id", "ic50_nm"]]
        df = df.merge(raw, on="candidate_id", how="left")

    df = df[df["label"].astype(str).str.upper() == "REAL"].copy()
    ...
    if len(df) < 5:
        print("  [SKIP] Not enough REAL candidates with IC50 data.")
        return
    ...
    if rho < 0 and pvalue < 0.05:
        print("  Negative rho = higher score -> " "lower IC50 -> biologically valid.")
    elif pvalue >= 0.05:
        print(
    ...
    _save_plot(ANALYSIS_DIR / "groupe09_patient_real_spearman.png")
...
def biological_interpretation(
    ...
    all_feats = set(imp_rf.index) | set(imp_lr.index) | set(imp_perm.index)
    n = len(all_feats)
    ...
    avg_rank = {
        f: np.mean([rank_rf.get(f, n), rank_lr.get(f, n), rank_perm.get(f, n)])
        for f in all_feats
    }
...
def main() -> None:
    ...
    print_importances(imp_rf, "FEATURE IMPORTANCE — Random Forest (Gini)")
    print_importances(imp_lr, "FEATURE IMPORTANCE — Logistic Regression (coefficients)")
    print_importances(imp_perm, "FEATURE IMPORTANCE — Permutation importance")
    ...
    if PATIENT_REAL.exists():
        ...
        if not X_real_raw.empty:
            evaluate_real_vs_decoy(rf, df_real, feature_names)
            evaluate_ic50_correlation(rf, df_real, feature_names)
    else:
