...
pipeline_data = {
    ...
    "CAND_07": {
        "label": "MEDIOCRE",
        "C_total_binding": 3.4,
        "C_binding_quality": 0.507,
    },
    "CAND_08": {
        "label": "MEDIOCRE",
        "C_total_binding": 1.2,
        "C_binding_quality": 0.564,
    },
    "CAND_09": {
        "label": "MEDIOCRE",
        "C_total_binding": 2.6,
        "C_binding_quality": 0.620,
    },
...
df_valid["netmhcpan_rank"] = (
    df_valid["netmhcpan_%rank"].rank(ascending=True).astype(int)
)
...
rho, pvalue = spearmanr(df_corr["pipeline_rank"], df_corr["netmhcpan_rank"])
...
print("\n── Tableau comparatif ──")
print(
    f"{'ID':<10} {'Label':<10} {'Pipeline':>10} {'NetMHCpan':>10} "
...
for _, row in df_sorted.iterrows():
    rank_str = (
        f"{int(row['netmhcpan_rank'])}" if pd.notna(row["netmhcpan_rank"]) else "N/A"
    )
    rank_el_str = (
        f"{row['netmhcpan_%rank']:.3f}" if pd.notna(row["netmhcpan_%rank"]) else "N/A"
    )
    delta_str = f"{int(row['delta'])}" if pd.notna(row["delta"]) else "N/A"
    print(
        f"{row['candidate_id']:<10} {row['label']:<10} "
...
for _, row in df_corr.iterrows():
    ...
    ax.scatter(
        row["pipeline_rank"], row["netmhcpan_rank"], color=color, s=100, zorder=3
    )
    ax.annotate(
        row["candidate_id"],
...
ax.legend(
    handles=[
        plt.Line2D(
            [0], [0], marker="o", color="w", markerfacecolor=c, markersize=8, label=lbl
        )
        for lbl, c in colors.items()
    ]
    + [
        plt.Line2D(
            [0], [0], linestyle="--", color="k", alpha=0.3, label="Perfect agreement"
        )
    ],
    fontsize=7,
    loc="upper left",
...
bar_colors = [colors.get(lbl, "blue") for lbl in df_delta["label"]]
